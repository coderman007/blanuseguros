<x-form-section submit="updateProfileInformation">
    ...
    <x-slot name="form">
        ...
        <div x-data="{photoName: null, photoPreview: null}" class="col-span-6 sm:col-span-4">
            ...
            <x-secondary-button class="bg-slate-400 hover:bg-[#253D5C] hover:text-slate-200 mt-2 mr-2" type="button" x-on:click.prevent="$refs.photo.click()">
                {{ __('Selecciona una nueva foto') }}
            </x-secondary-button>

            @if ($this->user->profile_photo_path)
            <x-secondary-button type="button" class="mt-2 bg-slate-400 hover:bg-[#253D5C] hover:text-slate-200" wire:click="deleteProfilePhoto">
                {{ __('Quitar Foto') }}
            </x-secondary-button>
            @endif
    ...
    <x-slot name="actions">
        <x-action-message class="mr-3 text-xl text-[#b2c7f0]" on="saved">
            {{ __('Guardado!') }}
        </x-action-message>
