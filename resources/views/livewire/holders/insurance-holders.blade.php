        ...
        <div class="grid items-center w-full md:grid-cols-12 mt-2">
            ...
            <div class="inline mt-4 pl-4 pr-24 md:pl-0 md:pr-0 md:mt-0 md:block md:col-span-4">
                <div class="text-xl font-bold text-center text-blue-400 uppercase">
                    <h1 class="text-red-800 dark:text-red-700">Titulares de<span class="text-gray-800 dark:text-gray-400"> Pólizas</span></h1>
                </div>
            </div>
            <div class="inline mt-4 md:mt-0 md:block md:col-span-4">
                <livewire:holders.insurance-holder-create />
            </div>
        </div>
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    ...
                    <tr class="text-center bg-white border-b text-md dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4 dark:text-lg">{{ $holder->name }}</td>
                        <td class="px-6 py-4 ">{{ $holder->email }}</td>
                        <td class="px-6 py-4 ">{{ $holder->phone }}</td>
                        <td class="px-6 py-4 dark:text-lg {{ $colorStatus }}">{{ $holder->is_active ? 'Activo' : 'Inactivo' }}</td>
                        <td class="flex justify-around py-4 pl-2 pr-8">
                            <div @if ($open) class="flex pointer-events-none opacity-20" @else class="flex" @endif>
                                <livewire:holders.insurance-holder-show :holder="$holder" :key="time() . $holder->id" />
                                <livewire:holders.insurance-holder-edit :holder="$holder" :key="time() . $holder->id" />
