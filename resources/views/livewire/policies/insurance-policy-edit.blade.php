<div class="bg-gray-300">
    ...
    <x-dialog-modal wire:model="openEdit">
        <x-slot name="title">
            <h2
                class="mt-3 text-2xl text-center text-yellow-500 hover:text-white w-full hover:bg-[#295083] bg-[#173152] p-4 rounded-2xl font-bold border-2 border-gray-100">
                Editar Póliza</h2>
        </x-slot>
        ...
        <x-slot name="content">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select
                        class="w-full border-2 border-yellow-700 shadow-sm shadow-yellow-700 rounded-md bg-yellow-200"
                        wire:model="insuranceCompanyId">
                        <option value="" class="hidden">Selecciona una compañía de seguros</option>
                        @foreach ($companies as $company)
                            <option class="text-[#173152] bg-yellow-100 text-lg" value="{{ $company->id }}">
                                {{ $company->name }}</option>
                        @endforeach
                    </select>
                    ...
                    <select
                        class="w-full bg-yellow-200 border-2 border-yellow-700 shadow-sm shadow-yellow-700 rounded-md"
                        wire:model="insuranceLineId">
                        <option value="" class="hidden">Selecciona un ramo</span></option>
                        @foreach ($lines as $line)
                            <option class="text-[#173152] bg-yellow-100 text-lg" value="{{ $line->id }}">
                                {{ $line->name }}</option>
                        @endforeach
                    </select>
                    ...
                    @if ($insuranceLineId && $lines->find($insuranceLineId)->name == 'Seguro de Automóvil')
                        ...
                        <x-input class="w-full border-2 border-yellow-700 shadow-md shadow-yellow-700" type="text"
                    ...
                    @if (
                        $insuranceLineId &&
                            in_array($lines->find($insuranceLineId)->name, ['Seguro de Responsabilidad Civil', 'Seguro de Cumplimiento']))
                        <x-label value="Número de Contrato" class="pb-2 text-gray-700 hover:text-yellow-700" />
                        <x-input class="w-full border-2 border-yellow-700 shadow-md shadow-yellow-700" type="text"
                            wire:model.defer="contractNumber" />
                    ...
                    <select
                        class="bg-yellow-200 w-full border-2 border-yellow-700 shadow-sm shadow-yellow-700 rounded-md"
                        wire:model="policyHolderId">
                        <option value="" class="hidden">Selecciona un tomador de póliza</option>
                        @foreach ($holders as $holder)
                            <option class="text-[#173152] bg-yellow-100 text-lg" value="{{ $holder->id }}">
                                {{ $holder->first_name . ' ' . $holder->last_name }}</option>
                        @endforeach
                    </select>
                    ...
                    <select
                        class="w-full bg-yellow-200 border-2 border-yellow-700 shadow-md shadow-yellow-700 rounded-md"
                        wire:model="paymentMethod">
                        <option value="" class="hidden">Selecciona una forma de pago</option>
                        <option value="Credit Card" class="text-[#173152] bg-yellow-100 text-lg">Tarjeta de Crédito
                        </option>
                        <option value="Debit Card" class="text-[#173152] bg-yellow-100 text-lg">Tarjeta de Débito
                        </option>
                        <option value="Bank Transfer" class="text-[#173152] bg-yellow-100 text-lg">Transferencia
                            Bancaria</option>
                    ...
                    <x-input class="w-full border-2 border-yellow-700 shadow-md shadow-yellow-700" type="number"
                        min="0" wire:model="netPremium" />
                    <x-input-error for="netPremium" />

                    <!-- Porcentaje de IVA -->
                    <x-label value="Porcentaje de IVA" class="pb-2 text-gray-700 hover:text-yellow-700" />
                    <x-input class="w-full border-2 border-yellow-700 shadow-md shadow-yellow-700" type="number"
                        min="0" max="100" wire:model="valueAddedTax" />
                ...
                <div>
                    ...
                    <x-input class="w-full border-2 border-yellow-700 shadow-md shadow-yellow-700" type="date"
                    ...
                    @if ($valueAddedTax > 0)
                        ...
                        <x-input class="w-full border-2 border-yellow-700 shadow-md shadow-yellow-700" type="number"
                            min="0" wire:model="valueAddedTaxAmount" readonly />
                ...
                <div>
                    ...
                    <div class="flex items-center">
                        ...
                        <span
                            class="{{ $isActive ? 'text-green-500' : 'text-red-500' }} text-lg">{{ $isActive ? 'Vigente' : 'Vencida' }}</span>
                    </div>
                ...
                <div>
                    ...
                    @if ($totalValue > 0)
                        ...
                        <x-input
                            class="w-full border-2 text-3xl border-green-700 shadow-md shadow-green-700 bg-green-100 text-center text-green-700"
                            type="number" min="0" wire:model="totalValue" readonly />
        ...
        <x-slot name="footer">
            <div class="mx-auto">
                <x-secondary-button
                    class="bg-gray-700 hover:bg-gray-500 text-yellow-500 hover:text-white hover:shadow-md hover:shadow-yellow-700 py-4 px-10 m-4"
                    wire:click="$set('openEdit', false)">
                    Salir
                </x-secondary-button>
                <x-secondary-button
                    class="bg-[#173152] hover:bg-[#295083] text-yellow-500 hover:text-white hover:shadow-md hover:shadow-yellow-700 py-4 px-10 m-4"
