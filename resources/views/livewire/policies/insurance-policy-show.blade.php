<div class="relative inline-block text-center cursor-pointer group">
    <a href="#" wire:click="$set('open', true)">
        ...
        <div
            class="absolute z-10 px-3 py-2 mb-2 text-center text-white bg-gray-800 rounded-lg opacity-0 pointer-events-none text-md group-hover:opacity-80 dark:bg-gray-600 dark:hover:bg-gray-700 bottom-full -left-3">
            Ver
    ...
    <x-dialog-modal wire:model="open">
        <x-slot name="title">
            <h5 class="mb-4 mt-4 text-3xl font-semibold tracking-tight text-center text-gray-900 dark:text-white">
                ...
                @elseif (
                    $policy->insuranceLine->name == 'Seguro de Cumplimiento' ||
                        $policy->insuranceLine->name == 'Seguro de Responsabilidad Civil')
        ...
        <x-slot name="content">
            <div class="bg-gray-100 dark:bg-gray-900 p-8 rounded-lg shadow-md">
                <div class="text-center mb-6">
                    <img src="{{ asset('storage/' . $policy->policyHolder->image) }}"
                        alt="{{ $policy->policyHolder->first_name . $policy->policyHolder->last_name }}"
                        class="w-32 h-32 mx-auto mb-4 rounded-full border-4 border-blue-600">
                    <h5 class="mt-4 text-2xl font-semibold">{{ $policy->policyHolder->first_name }}
                        {{ $policy->policyHolder->last_name }}</h5>
                ...
                <div class="mb-8">
                    ...
                    <p><strong class="text-blue-600">Documento de Identidad:</strong>
                        {{ $policy->policyHolder->document }}</p>
                ...
                <div class="mb-8">
                    <h5
                        class="mb-4 mt-4 text-3xl font-semibold tracking-tight text-center text-gray-900 dark:text-white">
                        Beneficiarios
                    </h5>
                            ...
                            <li class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                                <div class="relative w-full h-32 mb-4 overflow-hidden">
                                    <img src="{{ asset('storage/' . $beneficiary->image) }}"
                                        alt="{{ $beneficiary->name }}" class="w-full h-full object-cover rounded-lg">
                                </div>
