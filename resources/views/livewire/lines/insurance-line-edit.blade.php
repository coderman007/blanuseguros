                    ...
                    <div class="relative mt-4">
                        <label class="flex flex-col items-center justify-center h-48 gap-4 p-6 mx-auto bg-white border-2 border-gray-300 border-dashed rounded-lg shadow-md cursor-pointer w-72">
                            ...
                            <div class="absolute top-0 h-48 w-72">
                                ...
                                @else
                                <img class="object-cover w-full h-full rounded-lg" src="{{ asset('storage/' . $line->image) }}" class="mb-4" alt="Image">
                                @endif
                            </div>
                        </label>
                    </div>

                    <!-- Dropdown para Compañía -->
                    <x-label for="line.insurance_company_id" value="Compañía" class="text-gray-700" />
                    <select class="w-full rounded-md" required wire:model="line.insurance_company_id">
                        <option value="" class="hidden">Selecciona una compañía</option>
                        @foreach ($companies as $company)
                        <option value="{{ $company->id }}">{{ $company->name }}</option>
                        @endforeach
                    </select>
                    <x-input-error for="line.insurance_company_id" />
