    ...
    <x-dialog-modal wire:model="open">
        ...
        <x-slot name="content">
                    ...
                    <div class="absolute top-0 h-48 w-72">
                        @if ($image)
                            <img class="object-cover w-full h-full rounded-lg" src="{{ $image->temporaryUrl() }}"
                                class="mb-4" alt="Imagen de Compañía">
                        @else
                            <img class="object-cover w-full h-full rounded-lg"
                                src="{{ asset('storage/' . $company->image) }}" class="mb-4"
                                alt="Imagen {{ $company->name }}">
                        @endif
                    </div>
                </label>
            </div>
            ...
            <div class="flex items-center">
                ...
                <span
                    class="{{ $is_active ? 'text-green-500' : 'text-red-500' }} text-lg">{{ $is_active ? 'Activa' : 'Inactiva' }}</span>
            </div>
        ...
        <x-slot name="footer">
            <div class="flex justify-end mt-4">
                ...
                <x-secondary-button
                    ...
                    wire:click="update" wire:loading.attr="disabled" wire:target="update">
                    Actualizar
                </x-secondary-button>
            </div>
        </x-slot>
