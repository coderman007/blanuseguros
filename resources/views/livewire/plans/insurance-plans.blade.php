        ...
        <div class="grid items-center w-full md:grid-cols-12 mt-2">
            ...
            <div class="inline mt-4 pl-4 pr-24 md:pl-0 md:pr-0 md:mt-0 md:block md:col-span-4">
                <div class="text-xl font-bold text-center text-blue-400 uppercase">
                    <h1 class="text-red-800 dark:text-red-700">Planes de<span class="text-gray-800 dark:text-gray-400"> Seguro</span></h1>
                </div>
            </div>
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-sm text-center text-gray-100 uppercase bg-gray-400 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 cursor-pointer" wire:click="order('coverage')">
                            Cobertura
                            @if ($sort == 'coverage')
                            @if ($direction == 'asc')
                            <i class="ml-2 fa-solid fa-arrow-up-z-a"></i>
                        ...
                        <th scope="col" class="px-6 py-3 cursor-pointer" wire:click="order('line_id')">
                            Ramo
                            @if ($sort == 'line_id')
                            @if ($direction == 'asc')
                            <i class="ml-2 fa-solid fa-arrow-up-z-a"></i>
                ...
                <tbody>
                    ...
                    <tr class="text-center bg-white border-b text-md dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4 ">{{ $plan->coverage }}</td>
                        <td class="px-6 py-4 ">{{ $plan->price }}</td>
                        <td class="px-6 py-4 dark:text-lg">{{ $plan->is_active ? 'Activo' : 'Inactivo' }}</td>
                        <td class="px-6 py-4 dark:text-lg">{{ $plan->line->name }}</td>
                        <td class="flex justify-around py-4 pl-2 pr-8">
                            <div @if ($open) class="flex pointer-events-none opacity-20" @else class="flex" @endif>
                                ...
                                <a href="#" wire:click="confirmDelete({{ $plan->id }})">
