    ...
    <x-dialog-modal wire:model="open_edit">
        ...
        <x-slot name="content">
            ...
            <form wire:submit.prevent="update">
                ...
                <div class="relative mt-4">
                    <label class="flex flex-col items-center justify-center h-48 gap-4 p-6 mx-auto bg-white border-2 border-gray-300 border-dashed rounded-lg shadow-md cursor-pointer w-72">
                        <div class="flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="#173152" viewBox="0 0 24 24" class="w-16 h-16 text-gray-600">
                ...
                <!-- Dropdown para Ramo -->
                <x-label for="plan.insurance_line_id" value="Ramo" class="text-gray-700" />
                <select class="w-full rounded-md" required wire:model="plan.insurance_line_id">
                    <option value="" class="hidden">Selecciona un ramo</option>
                    @foreach ($lines as $line)
                    <option value="{{ $line->id }}">{{ $line->name }}</option>
                    @endforeach
                </select>
                <x-input-error for="plan.insurance_line_id" />
