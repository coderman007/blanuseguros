<div class="relative inline-block text-center cursor-pointer group">
    ...
    <x-dialog-modal wire:model="open">
        <x-slot name="title">
            <h2 class="mt-3 text-2xl text-center">Editar Plan</h2>
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="relative mt-4">
                <label class="flex flex-col items-center justify-center h-48 gap-4 p-6 mx-auto bg-white border-2 border-gray-300 border-dashed rounded-lg shadow-md cursor-pointer w-72">
                    ...
                    <div class="text-center">
                        <span class="font-normal text-gray-600">Cambiar imagen del plan (opcional)</span>
                    </div>
                    ...
                    <div class="absolute top-0 h-48 w-72">
                        ...
                        @else
                        <img class="object-cover w-full h-full rounded-lg" src="{{ asset('storage/' . $plan->image) }}" class="mb-4" alt="Image">
                        @endif
                    </div>
                </label>
            </div>
            ...
            <select class="w-full rounded-md" required wire:model="lineId">
                <option value="" disabled>Selecciona un ramo</option>
                @foreach ($lines as $line)
                <option value="{{ $line->id }}">{{ $line->name }}</option>
                @endforeach
            </select>
            <x-input-error for="lineId" />

            <!-- Nombre -->
            <div class="mt-4">
                <x-label value="Nombre" class="text-gray-700" />
                <x-input type="text" class="w-full" required wire:model.defer="name" />
        ...
        <x-slot name="footer">
            <div class="flex justify-end mt-4">
                ...
                <x-secondary-button class="text-blue-500 border border-blue-500 shadow-lg hover:shadow-blue-400 disabled:opacity-50 disabled:bg-blue-600 disabled:text-white" wire:click="updatePlan" wire:loading.attr="disabled" wire:target="updatePlan">
                    Editar
                </x-secondary-button>
            </div>
        </x-slot>
